...
If this repo ever splits into genuinely separate contexts, the multi-context layout is a root `CONTEXT-MAP.md` pointing at one `CONTEXT.md` per context, each with its own `docs/adr/`. Until that file exists, treat the repo as single-context.
...
> _Contradicts ADR-0007 (streaming via edge runtime), but worth reopening because…_
